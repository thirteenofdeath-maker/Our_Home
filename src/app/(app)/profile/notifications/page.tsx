...
export default async function NotificationSettingsPage() {
  ...
  const [{ data: preferences }, { data: subscriptions }, { data: config, error: configError }] = await Promise.all([
    supabase.from("notification_preferences").select("plan_enabled,pets_enabled,finance_enabled,day_before_enabled,due_day_enabled,member_birthdays_enabled,pet_birthdays_enabled,birthday_week_before_enabled,digest_mode_enabled,daily_digest_enabled,weekly_digest_enabled").eq("user_id", user.id).maybeSingle(),
