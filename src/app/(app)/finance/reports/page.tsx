...
import {
  FinanceDonutChart,
  type DonutSegment,
} from "@/features/finance/components/FinanceDonutChart";
...
const DONUT_COLORS = [
  ...
  "stroke-finance-warning",
  "stroke-finance-transfer",
  "stroke-finance-income",
];
...
export default async function ReportsPage({
  ...
  const view: View =
    params.view === "month" || params.view === "compare"
      ? params.view
      : "category";
  const type: ReportType = params.type === "INCOME" ? "INCOME" : "EXPENSE";
  const scope: "PERSONAL" | "HOUSEHOLD" =
    params.scope === "HOUSEHOLD" && household ? "HOUSEHOLD" : "PERSONAL";
  ...
  const range =
    view === "category"
      ...
      : {
          start: financeMonthRange(
            shiftFinanceMonth(currentFinanceMonth(), 1 - monthsCount),
          ).start,
          end: financeMonthRange(currentFinanceMonth()).end,
        };
  ...
  const monthPickerQuery = {
    view,
    type,
    months: String(monthsCount),
    ...(scope === "HOUSEHOLD" ? { scope: "HOUSEHOLD" } : {}),
  };
        ...
        options={[
          {
            value: "category",
            label: "หมวดหมู่",
            href: buildHref(commonParams, { view: "category" }),
          },
          {
            value: "month",
            label: "เดือน",
            href: buildHref(commonParams, { view: "month" }),
          },
          {
            value: "compare",
            label: "เปรียบเทียบ",
            href: buildHref(commonParams, { view: "compare" }),
          },
        ]}
      />
          ...
          options={[
            {
              value: "PERSONAL",
              label: "ส่วนตัว",
              href: buildHref(commonParams, { scope: undefined }),
            },
            {
              value: "HOUSEHOLD",
              label: "ครอบครัว",
              href: buildHref(commonParams, { scope: "HOUSEHOLD" }),
            },
          ]}
        />
      ) : null}
      ...
      {view === "category" ? (
        <>
          <FinanceMonthNavigator
            ...
            previousMonth={shiftFinanceMonth(month, -1)}
            nextMonth={shiftFinanceMonth(month, 1)}
            currentMonth={currentMonth}
            pathname="/finance/reports"
            query={monthPickerQuery}
          />
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            {report.months.length ? (
              report.months.map((m) => (
                <div
                  key={m.currency}
                  className="col-span-3 grid w-full grid-cols-3 gap-2 rounded-[1.25rem] bg-finance-surface-strong p-3"
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate font-semibold tabular-nums text-finance-expense">
                      {formatCurrency(m.expense, m.currency)}
                    </p>
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs text-finance-muted">รายรับรวม</p>
                    <p className="truncate font-semibold tabular-nums text-finance-income">
                      {formatCurrency(m.income, m.currency)}
                    </p>
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs text-finance-muted">สุทธิ</p>
                    <p className="truncate font-semibold tabular-nums text-finance-text">
                      {formatCurrency(
                        subtractMoney(m.income, m.expense),
                        m.currency,
                      )}
                    </p>
                  </div>
              ...
              <div className="col-span-3">
                <FinanceEmptyState
                  ...
                  description="เมื่อมีรายการ ตัวเลขจะแสดงที่นี่"
                />
              </div>
            )}
          </div>
            ...
            options={[
              {
                value: "EXPENSE",
                label: "รายจ่าย",
                href: buildHref(commonParams, { type: "EXPENSE" }),
              },
              {
                value: "INCOME",
                label: "รายรับ",
                href: buildHref(commonParams, { type: "INCOME" }),
              },
            ]}
          />

          <CategoryReport
            categories={report.categories.filter((c) => c.type === type)}
            type={type}
          />
        </>
      ) : null}
        ...
        <>
            ...
            options={[
              {
                value: "3",
                label: "3 เดือน",
                href: buildHref(commonParams, { months: "3" }),
              },
              {
                value: "6",
                label: "6 เดือน",
                href: buildHref(commonParams, { months: "6" }),
              },
              {
                value: "12",
                label: "12 เดือน",
                href: buildHref(commonParams, { months: "12" }),
              },
            ]}
          />
          {report.months.length === 0 ? (
            <FinanceEmptyState
              ...
              description="ยังไม่มีรายการในช่วงเวลานี้"
            />
          ) : (
            <div className="flex flex-col gap-2">
              {report.months.map((m) => (
                <div
                  ...
                  className="flex items-center justify-between rounded-[1.25rem] bg-finance-surface-strong p-3"
                >
      ...
      {view === "compare" ? (
        <>
          ...
          {report.months.length === 0 ? (
            ...
            Array.from(new Set(report.months.map((m) => m.currency))).map(
              (currency) => (
                <CompareChart
                  key={currency}
                  currency={currency}
                  months={report.months.filter((m) => m.currency === currency)}
                />
              ),
            )
          )}
  ...
  if (categories.length === 0) {
    return (
      <FinanceEmptyState
        ...
        title={
          type === "EXPENSE"
            ? "ยังไม่มีรายจ่ายตามหมวดหมู่"
            : "ยังไม่มีรายรับตามหมวดหมู่"
        }
    ...
    <div className="flex flex-col gap-4">
      {[...byCurrency.entries()].map(([currency, rows]) => {
        ...
        return (
          <div
            key={currency}
            className="flex flex-col gap-3 rounded-[1.25rem] bg-finance-surface-strong p-4"
          >
            {byCurrency.size > 1 ? (
            ...
            <div className="flex flex-col gap-2">
              {sortedRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`size-2.5 shrink-0 rounded-full ${DONUT_COLORS[index % DONUT_COLORS.length]!.replace("stroke-", "bg-")}`}
                    />
                    <span className="min-w-0 flex-1 truncate text-sm text-finance-text">
                      {row.name}
                    </span>
                    <span className="shrink-0 text-xs text-finance-muted">
                      {percent.toFixed(0)}%
                    </span>
                    <span className="shrink-0 text-sm font-medium tabular-nums text-finance-text">
    ...
    <div className="flex flex-col gap-3 rounded-[1.25rem] bg-finance-surface-strong p-4">
        ...
        {months.map((m) => (
          ...
          >
            <div className="flex h-full w-full items-end gap-0.5">
              <div
                className="w-full rounded-t-sm bg-finance-expense/70"
                style={{
                  height: `${Math.max((Number(m.expense) / maxValue) * 100, 2)}%`,
                }}
              />
              <div
                className="w-full rounded-t-sm bg-finance-income/70"
                style={{
                  height: `${Math.max((Number(m.income) / maxValue) * 100, 2)}%`,
                }}
              />
            </div>
      ...
      <div className="flex items-center gap-4 text-xs text-finance-muted">
        <span className="flex items-center gap-1">
          <span className="size-2 rounded-full bg-finance-expense/70" /> รายจ่าย
        </span>
        <span className="flex items-center gap-1">
          <span className="size-2 rounded-full bg-finance-income/70" /> รายรับ
        </span>
      </div>
    </div>
  );
