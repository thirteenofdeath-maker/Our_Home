...
export default async function Page({
  ...
  const o = await getInstallmentOccurrence(supabase, occurrenceId);
  if (!o || o.status !== "OPEN") notFound();
  const p = await getInstallmentPlan(supabase, o.planId);
  if (!p) notFound();
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-xl font-semibold">
        จ่ายงวด {o.sequenceNumber}: {p.name}
      </h1>
      <PayInstallmentForm
        id={o.id}
