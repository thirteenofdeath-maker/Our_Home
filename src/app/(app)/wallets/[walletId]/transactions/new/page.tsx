...
export default async function NewTransactionPage({
  ...
  const safeReturnTo =
    returnTo === FINANCE_RETURN_TO ? FINANCE_RETURN_TO : undefined;
  ...
  const templateApplies =
    ...
    (template.scope === "PERSONAL"
      ? template.ownerUserId === wallet.owner_user_id
      : template.householdId === wallet.household_id);
  ...
  if (templateApplies && template) {
    ...
    if (template.categoryId && template.categoryArchived) {
      staleNotices.push(
        "หมวดหมู่ที่บันทึกไว้ถูก Archive แล้ว กรุณาเลือกหมวดหมู่ใหม่ก่อนบันทึก",
      );
    }
  }
  ...
  const occurrence = occurrenceId
    ? await getOccurrence(supabase, occurrenceId)
    : null;
  const occurrenceApplies =
    occurrence &&
    occurrence.status === "UPCOMING" &&
    occurrence.transactionType === transactionType &&
    (occurrence.scope === "PERSONAL"
      ? occurrence.ownerUserId === wallet.owner_user_id
      : occurrence.householdId === wallet.household_id);
  ...
  return (
    <div className="mx-auto flex w-full max-w-xl flex-col gap-4">
      <PageHeader
        title={transactionType === "INCOME" ? "เพิ่มรายรับ" : "เพิ่มรายจ่าย"}
        backHref={`/wallets/${walletId}`}
      />
      <TransactionForm
        walletId={walletId}
