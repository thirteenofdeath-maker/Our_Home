...
export default async function PetDetailPage({ params }: PageProps<"/pets/[petId]">) {
  ...
  return <div className="finance-scope -mx-4 -mt-2 flex min-w-0 flex-col gap-5 px-4 pb-8 pt-3">
    ...
    {!pet.archived_at && role !== "observer" ? <FormSheetButton
    ...
    <section className="flex flex-col gap-3"><div><p className="text-sm text-finance-muted">สุขภาพและการดูแล</p><h2 className="font-semibold text-finance-text">ไทม์ไลน์ของ {pet.name}</h2></div><PetCareTimeline petId={pet.id} records={records} userId={user.id} canManageAll={canManage}/></section>
