...
export default async function PetDetailPage({ params }: PageProps<"/pets/[petId]">) {
  ...
  return <div className="flex flex-col gap-4"><Card className="flex flex-col items-center gap-3 text-center"><PetPhoto name={pet.name} url={pet.photoUrl} size="lg"/><div><h1 className="text-xl font-semibold">{pet.name}</h1><p className="text-foreground-muted">{SPECIES_LABEL[pet.species]}{pet.breed?` · ${pet.breed}`:""}</p></div></Card>
