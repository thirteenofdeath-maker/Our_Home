...
export default async function PetDetailPage({
  ...
  return (
    <div className="finance-scope -mx-4 -mt-2 flex min-w-0 flex-col gap-5 px-4 pb-8 pt-3">
      ...
      <Card className="flex flex-col items-center gap-3 rounded-[1.5rem] bg-finance-surface-strong text-center">
        <PetPhoto name={pet.name} url={pet.photoUrl} size="lg" />
        <div>
        ...
        {latestWeight ? (
          <div className="rounded-full bg-finance-primary-soft px-4 py-2 text-sm font-medium text-finance-primary-strong">
            น้ำหนักล่าสุด {latestWeight.value} {latestWeight.unit}
          </div>
        ) : null}
      </Card>
      <Card className="flex flex-col gap-2 rounded-[1.5rem] bg-finance-surface-strong text-finance-text">
        <p>เพศ: {pet.sex ? SEX_LABEL[pet.sex] : "ไม่ระบุ"}</p>
        <p>
          วันเกิด: {pet.birthday ?? "ไม่ระบุ"}
          {age !== null ? ` · อายุ ${age} ปี` : ""}
        </p>
        <p>สถานะ: {pet.archived_at ? "เก็บเข้าคลัง" : "ใช้งาน"}</p>
        <div>
          <p className="font-medium">ผู้ดูแล</p>
          <p className="text-finance-muted">
            {pet.caregivers.length
              ? pet.caregivers
                  .map(
                    (item) => item.profile?.display_name || item.profile?.email,
                  )
                  .join(", ")
              : "ยังไม่ระบุ"}
          </p>
      ...
      <section className="flex flex-col gap-3">
        <div>
          <p className="text-sm text-finance-muted">สุขภาพและการดูแล</p>
          <h2 className="font-semibold text-finance-text">
            ไทม์ไลน์ของ {pet.name}
          </h2>
        </div>
        <PetCareTimeline
          petId={pet.id}
      ...
      {canManage ? (
        <div className="flex flex-col gap-2">
          ...
          <form action={archivePetAction}>
            <input type="hidden" name="petId" value={pet.id} />
            <input
              type="hidden"
              name="archived"
              value={pet.archived_at ? "false" : "true"}
            />
            <button className={buttonClassName("primary", "md")} type="submit">
              {pet.archived_at ? "นำกลับมาใช้งาน" : "เก็บเข้าคลัง"}
            </button>
          </form>
        </div>
      ) : null}
