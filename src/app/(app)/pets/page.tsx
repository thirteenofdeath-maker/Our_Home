  ...
  return (
    <div className="flex flex-col gap-5">
      <TopLevelCover
        ...
        title="สัตว์เลี้ยง"
        description={`${active.length} ชีวิตตัวน้อยที่ทำให้บ้านเป็นบ้าน`}
        icon="pets"
        tone="pets"
      />
      ...
      <section className="flex flex-col gap-3">
        {active.length ? (
          active.map((pet) => <PetCard key={pet.id} pet={pet} />)
        ) : (
          <Card>
            <p className="text-sm text-foreground-muted">ยังไม่มีสัตว์เลี้ยง</p>
          </Card>
        )}
      </section>
      {archived.length ? (
        <section className="flex flex-col gap-3">
          <h2 className="font-semibold">เก็บเข้าคลัง</h2>
          {archived.map((pet) => (
