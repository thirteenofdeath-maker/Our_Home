...
export default async function PetsPage() {
  ...
  return <div className="flex flex-col gap-5"><header className="flex items-center justify-between gap-3"><div><p className="text-sm text-foreground-muted">{household.name}</p><h1 className="text-xl font-semibold">สัตว์เลี้ยง</h1></div>{canManage ? <Link href="/pets/new" className={buttonClassName("primary","md","w-auto px-4")}>+ เพิ่มสัตว์เลี้ยง</Link> : null}</header>
