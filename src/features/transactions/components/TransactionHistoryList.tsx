...
function isTransfer(item: TransactionHistoryItem): boolean {
  return (
    ...
    Boolean(item.pocketTransfer) ||
    Boolean(item.walletTransfer)
  );
}
 ...
 */
function rowVisual(item: TransactionHistoryItem): {
  icon: AppIconName;
  className: string;
} {
  if (item.voidedAt)
    return {
      icon: isTransfer(item)
        ? "transfer"
        : item.amount.startsWith("-")
          ? "expense"
          : "income",
      className: "bg-finance-muted/15 text-finance-muted",
    };
  if (item.adjustment)
    return {
      icon: "income",
      className: "bg-finance-income/15 text-finance-income",
    };
  if (isTransfer(item))
    return {
      icon: "transfer",
      className: "bg-finance-transfer/15 text-finance-transfer",
    };
  return item.amount.startsWith("-")
    ? {
        icon: "expense",
        className: "bg-finance-expense/15 text-finance-expense",
      }
  ...
  if (items.length === 0) {
    return (
      <EmptyState
        ...
        description="เพิ่มรายรับหรือรายจ่ายแรกของคุณ"
      />
    );
  }
  ...
  return (
    <ul
      className={cn(
        ...
        variant === "dashboard"
          ? "gap-1"
          : "divide-y divide-finance-primary-soft/60 overflow-hidden rounded-[1.25rem] bg-finance-surface-strong",
      )}
    >
      {items.map((item) => {
        ...
        const usingCategoryAsTitle =
          !transfer && !item.title && Boolean(item.categoryName);
        ...
        const context =
          variant === "wallet"
            ? item.pocketName
            : `${item.walletName} / ${item.pocketName}`;
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block truncate text-sm font-medium text-finance-text">
                  {title}
                  {item.voidedAt ? (
                    <span className="ml-2 text-xs font-normal text-finance-expense">
                      ยกเลิกแล้ว
                    </span>
                  ) : null}
                </span>
              ...
              >
                {formatCurrency(
                  item.pocketTransfer?.amount ?? item.amount,
                  item.currency ?? currency ?? "THB",
                )}
              </span>
            </Link>
          </li>
