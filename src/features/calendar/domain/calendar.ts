...
export const EVENT_SCOPES = [
  "PERSONAL",
  "HOUSEHOLD",
] as const satisfies readonly CalendarEventScope[];
...
export const calendarFormSchema = z
  ...
  .superRefine((value, ctx) => {
    if (value.scope === "PERSONAL" && value.participantIds.length)
      ctx.addIssue({
        code: "custom",
        message: "Personal events cannot have participants",
        path: ["participantIds"],
      });
    if (value.isAllDay) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(value.allDayDate))
        ctx.addIssue({
          code: "custom",
      ...
      const start = parseBangkokLocal(value.startsLocal);
      const end = value.endsLocal ? parseBangkokLocal(value.endsLocal) : null;
      if (!start)
        ctx.addIssue({
          code: "custom",
...
export function monthKey(value: string | undefined, now = new Date()): string {
...
export function monthDays(
  ...
  return Array.from({ length: cellCount }, (_, i) => {
    const d = new Date(Date.UTC(y, m - 1, i - offset + 1));
    const date = d.toISOString().slice(0, 10);
    return { date, inMonth: d.getUTCMonth() === m - 1 };
  });
}
