  ...
  if (!tasks.length) {
    return (
      <Card className="rounded-[1.25rem] bg-finance-surface-strong">
        <p className="text-sm text-finance-muted">ยังไม่มีงานในมุมมองนี้</p>
      </Card>
    );
  }
  return (
    <div className="flex flex-col gap-2">
      {tasks.map((task) => (
        <Card
          key={task.id}
          className="flex min-w-0 items-start gap-3 rounded-[1.25rem] bg-finance-surface-strong p-4"
        >
          <form action={togglePlanTaskAction}>
            ...
            <button
              ...
              aria-label={
                task.is_completed
                  ? `ทำให้ ${task.title} ยังไม่เสร็จ`
                  : `ทำ ${task.title} ให้เสร็จ`
              }
              className={cn(
          ...
          <Link href={`/calendar/tasks/${task.id}`} className="min-w-0 flex-1">
            <div className="flex min-w-0 items-start justify-between gap-2">
              <p
                className={cn(
                  "truncate font-medium text-finance-text",
                  task.is_completed && "text-finance-muted line-through",
                )}
              >
                {task.title}
              </p>
            ...
            <p className="mt-1 truncate text-xs text-finance-muted">
              {task.list_name} · {planDateLabel(task.due_date, task.due_time)}
            </p>
