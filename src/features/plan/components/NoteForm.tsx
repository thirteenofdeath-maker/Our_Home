  ...
  return (
    <form action={action} className="flex min-w-0 flex-col gap-4">
      ...
      <Field label="หัวข้อ" htmlFor="plan-note-title">
        <Input id="plan-note-title" name="title" maxLength={160} defaultValue={note?.title ?? ""} placeholder="หัวข้อโน้ต" />
      </Field>
      <Field label="ข้อความ" htmlFor="plan-note-content">
        <Textarea id="plan-note-content" name="content" maxLength={20000} defaultValue={note?.content ?? ""} className="h-48 resize-y" placeholder="เขียนสิ่งที่อยากจำ…" />
      </Field>
      <Field label="ขอบเขต" htmlFor="plan-note-scope">
        {note ? (
          <>
            <Input id="plan-note-scope" value={note.scope === "PERSONAL" ? "ส่วนตัว" : "ครอบครัว"} disabled />
            <input type="hidden" name="scope" value={note.scope} />
          </>
        ) : (
          <Select id="plan-note-scope" name="scope" value={scope} onChange={(event) => setScope(event.target.value as typeof scope)}>
      ...
      <Field label="สีโน้ต" htmlFor="plan-note-color">
        <Select id="plan-note-color" name="color" defaultValue={note?.color ?? "SAGE"}>
      ...
      <SubmitButton size="lg">{note ? "บันทึกการแก้ไข" : "เพิ่มโน้ต"}</SubmitButton>
    </form>
  );
}
