...
describe("FinanceCreateFlow — sheet state architecture", () => {
  ...
  it("uses one textual back action in the sheet header without an extra arrow control", () => {
    expect(flow).toContain('closeLabel="ย้อนกลับ"');
    expect(flow).toContain("goBack");
    expect(flow).not.toContain("ย้อนกลับไปเลือกประเภท");
  ...
  it("returns the unified transfer form directly to quick-create choices", () => {
    expect(flow).toMatch(
      /function goBack\(\) \{[\s\S]*?setStage\("choosing"\)/,
    );
...
describe("FinanceCreateFlow — reuses existing forms/writers, never forks them", () => {
  ...
  it("existing full-page routes still render the unmodified full-page variant of every reused form — routes were not deleted", () => {
    const routes: Array<[string, string]> = [
      [
        "src/app/(app)/wallets/[walletId]/transactions/new/page.tsx",
        "<TransactionForm",
      ],
      [
      ...
      [
        "src/app/(app)/wallets/[walletId]/transfer/wallet/page.tsx",
        "<WalletTransferForm",
      ],
      ["src/app/(app)/wallets/new/page.tsx", "<WalletForm"],
      [
        "src/app/(app)/wallets/[walletId]/pockets/new/page.tsx",
        "<AddPocketForm",
      ],
    ];
  ...
  it("no writer is duplicated — createIncomeExpenseAction/createPocketTransferAction/createWalletTransferAction/createWalletAction/createPocketAction are each defined in exactly one actions.ts file", () => {
    ...
    expect(
      (
        txActions.match(/export async function createIncomeExpenseAction/g) ??
        []
      ).length,
    ).toBe(1);
    expect(
      (
        txActions.match(/export async function createPocketTransferAction/g) ??
        []
      ).length,
    ).toBe(1);
    expect(
      (
        txActions.match(/export async function createWalletTransferAction/g) ??
        []
      ).length,
    ).toBe(1);
    expect(
      (walletActions.match(/export async function createWalletAction/g) ?? [])
        .length,
    ).toBe(1);
    expect(
      (pocketActions.match(/export async function createPocketAction/g) ?? [])
        .length,
    ).toBe(1);
    ...
    expect(flow).not.toMatch(
      /function createIncomeExpenseAction|function createPocketTransferAction|function createWalletTransferAction/,
    );
  });
