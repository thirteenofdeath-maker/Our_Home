...
const STATUS_BAR_CLASS: Record<ReturnType<typeof budgetStatus>, string> = {
  UNDER: "bg-finance-income",
  NEAR_LIMIT: "bg-finance-warning",
  OVER: "bg-finance-expense",
};
...
export function BudgetCard({
  ...
  const periodLabel = compact
    ...
    : new Intl.DateTimeFormat("th-TH", {
      ...
      }).format(new Date(`${item.periodMonth}T00:00:00+07:00`));

  return (
    <Link href={`/finance/budgets/${item.budgetId}`}>
      <Card
        className={`flex flex-col gap-3 rounded-[1.25rem] bg-finance-surface-strong shadow-sm ${item.archivedAt ? "opacity-60" : ""}`}
      >
        <div className="flex items-center justify-between">
          <p className="font-medium text-finance-text">
            {item.categoryName}
            {item.categoryArchived ? (
          ...
          {!compact ? (
            periodLabel ? (
              <span className="shrink-0 rounded-full bg-finance-primary-soft px-2 py-0.5 text-xs font-medium text-finance-muted">
                {periodLabel}
              </span>
            ) : (
        ...
        <div className="flex items-baseline justify-between text-sm">
          <span
            className={
              isOver ? "font-medium text-finance-expense" : "text-finance-text"
            }
          >
            {formatCurrency(item.netSpent, item.currency)}
          </span>
        ...
        <div
          className={`w-full overflow-hidden rounded-full bg-finance-primary-soft ${compact ? "h-2" : "h-2.5"}`}
        >
          <div
        ...
        {isOver ? (
          ...
          <p className="text-xs font-medium text-finance-expense">
            เกินงบ{" "}
            {formatCurrency(
              subtractMoney(item.netSpent, item.budgetAmount),
              item.currency,
            )}
          </p>
        ) : (
          <p className="text-xs text-finance-muted">
            {percent.toFixed(0)}% · เหลือ{" "}
