...
export function CreateInstallmentForm({
  ...
  /** Presentation only — this form never had its own card chrome, so both variants render identically. */
  variant?: "page" | "sheet";
}) {
  void variant;
    ...
    <form action={action} className="finance-ui-tone flex flex-col gap-4">
      <Field label="ขอบเขต" htmlFor="scope">
        <Select
          ...
          value={scope}
          onChange={(event) =>
            setScope(event.target.value as "PERSONAL" | "HOUSEHOLD")
          }
        >
      ...
      <Field label="ชื่อแผน" htmlFor="name">
        <Input id="name" name="name" required />
      </Field>
      <Field label="ยอดรวม" htmlFor="totalAmount">
        <Input
          id="totalAmount"
          name="totalAmount"
          inputMode="decimal"
      ...
      <Field label="จำนวนงวด" htmlFor="installmentCount">
        <Input
          id="installmentCount"
      ...
      <Field label="งวดแรก" htmlFor="startDate">
        <Input id="startDate" name="startDate" type="date" required />
      </Field>
      <Field label="ห่างกัน (เดือน)" htmlFor="intervalMonths">
        <Input
          id="intervalMonths"
          name="intervalMonths"
          type="number"
      ...
      <Field label="หมวดหมู่รายจ่าย" htmlFor="categoryId">
        <CategoryPicker
          key={scope}
          name="categoryId"
          categories={scope === "PERSONAL" ? personal : household}
          transactionType="EXPENSE"
          allowCreate={false}
        />
      </Field>
      {state.error ? <p className="text-danger">{state.error}</p> : null}
      <SubmitButton>สร้างแผนผ่อน</SubmitButton>
    </form>
  );
}
