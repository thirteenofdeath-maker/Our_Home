...
function node(
  ...
  return {
    ...
    children,
    is_system: isSystem,
    system_key: isSystem ? id : null,
    scope: isSystem ? null : "PERSONAL",
    owner_user_id: null,
    household_id: null,
...
describe("CategoryPicker", () => {
  ...
  const home = node("root-home", "บ้านและที่อยู่อาศัย", [
    node("child-rent", "ค่าเช่า"),
  ]);
  ...
  it("submits the exact selected UUID and renders grouped hierarchy", () => {
    const html = renderToStaticMarkup(
      createElement(CategoryPicker, {
        ...
        walletId: "wallet-1",
        defaultSelected: { id: meal.id, label: "" },
      }),
    );
    expect(html).toContain(
      'type="hidden" name="categoryId" value="child-meal"',
    );
  ...
  it("searches Thai child names while retaining their root group", () => {
    ...
    expect(filtered[0]?.children.map((item) => item.id)).toEqual([
      "child-rent",
    ]);
  });

  it("supports optional selection without offering creation when no wallet can scope it", () => {
    const html = renderToStaticMarkup(
      createElement(CategoryPicker, {
        name: "categoryId",
        categories: [food],
    ...
    expect(html).toContain('type="hidden" name="categoryId" value=""');
    expect(html).toContain("ไม่กำหนด");
    expect(html).not.toContain("+ เพิ่มหมวดหมู่ใหม่");
    expect(html).not.toContain("+ เพิ่มหมวดหมู่ย่อย");
  });
});
