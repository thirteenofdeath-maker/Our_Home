...
describe("notification digests", () => {
  ...
  it("builds one compact summary instead of many messages", () => {
    expect(
      digestBody(
        { tasks: 3, appointments: 1, birthdays: 0, bills: 2, pets: 1 },
        "DAILY",
      ),
    ).toBe("3 งาน · 1 นัดหมาย · 2 บิล · 1 รายการสัตว์เลี้ยง");
  });

  it("finds birthdays across the new-year boundary", () => {
    expect(birthdayFallsInWindow("2000-01-02", "2026-12-29", "2027-01-04")).toBe(true);
    expect(birthdayFallsInWindow("2000-02-02", "2026-12-29", "2027-01-04")).toBe(false);
  });
});
