...
language sql security invoker stable set search_path='' as $$
 with entry_summary as(select t.id,string_agg(distinct w.name,' | ')wallets,string_agg(distinct p.name,' | ')pockets,min(w.currency)currency,case when t.transaction_type='INCOME'then sum(e.amount)when t.transaction_type='EXPENSE'then sum(-e.amount)else max(abs(e.amount))end amount from public.transactions t join public.transaction_entries e on e.transaction_id=t.id join public.wallets w on w.id=e.wallet_id join public.pockets p on p.id=e.pocket_id group by t.id)
 ...
 from public.transactions t join entry_summary s on s.id=t.id left join public.categories c on c.id=t.category_id
